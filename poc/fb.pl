diagram_fact(align, cell_10, "left").
diagram_fact(as, cell_10, "geometry").
diagram_fact(as, cell_6, "geometry").
diagram_fact(as, cell_7, "geometry").
diagram_fact(as, cell_8, "geometry").
diagram_fact(as, cell_9, "geometry").
diagram_fact(aspect, cell_7, "fixed").
diagram_fact(aspect, cell_8, "fixed").
diagram_fact(aspect, cell_9, "fixed").
diagram_fact(cell, cell_10, "").
diagram_fact(cell, cell_4, "").
diagram_fact(cell, cell_5, "").
diagram_fact(cell, cell_6, "").
diagram_fact(cell, cell_7, "").
diagram_fact(cell, cell_8, "").
diagram_fact(cell, cell_9, "").
diagram_fact(diagram, diagram_1, "").
diagram_fact(fillColor, cell_10, "#f8cecc").
diagram_fact(fillColor, cell_6, "#dae8fc").
diagram_fact(fillColor, cell_7, "#fff2cc").
diagram_fact(fillColor, cell_8, "#d5e8d4").
diagram_fact(fillColor, cell_9, "#d5e8d4").
diagram_fact(gradientColor, cell_6, "#7ea6e0").
diagram_fact(height, cell_10, 160).
diagram_fact(height, cell_6, 212.5).
diagram_fact(height, cell_7, 40).
diagram_fact(height, cell_8, 40).
diagram_fact(height, cell_9, 40).
diagram_fact(horizontal, cell_6, 1).
diagram_fact(html, cell_10, 1).
diagram_fact(html, cell_6, 1).
diagram_fact(html, cell_7, 1).
diagram_fact(html, cell_8, 1).
diagram_fact(html, cell_9, 1).
diagram_fact(kind, cell_7, "ellipse").
diagram_fact(kind, cell_8, "ellipse").
diagram_fact(kind, cell_9, "ellipse").
diagram_fact(mxGraphModel, mxGraphModel_2, mxGraphModel_2).
diagram_fact(name, diagram_1, "Page-1").
diagram_fact(parent, cell_10, id_1).
diagram_fact(parent, cell_5, id_0).
diagram_fact(parent, cell_6, id_1).
diagram_fact(parent, cell_7, id_1).
diagram_fact(parent, cell_8, id_1).
diagram_fact(parent, cell_9, id_1).
diagram_fact(root, root_3, root_3).
diagram_fact(rounded, cell_10, 0).
diagram_fact(rounded, cell_6, 1).
diagram_fact(strokeColor, cell_10, "#b85450").
diagram_fact(strokeColor, cell_6, "#6c8ebf").
diagram_fact(strokeColor, cell_7, "#d6b656").
diagram_fact(strokeColor, cell_8, "#82b366").
diagram_fact(strokeColor, cell_9, "#82b366").
diagram_fact(synonym, cell_10, yHWN_XODvyVebYNQqBD7__8).
diagram_fact(synonym, cell_4, id_0).
diagram_fact(synonym, cell_5, id_1).
diagram_fact(synonym, cell_6, yHWN_XODvyVebYNQqBD7__2).
diagram_fact(synonym, cell_7, yHWN_XODvyVebYNQqBD7__3).
diagram_fact(synonym, cell_8, yHWN_XODvyVebYNQqBD7__4).
diagram_fact(synonym, cell_9, yHWN_XODvyVebYNQqBD7__5).
diagram_fact(value, cell_10, "&lt;div&gt;&lt;div&gt;&lt;div&gt;&lt;div&gt;&lt;div&gt;initially {⟪self.dirname = ''⟫}&lt;/div&gt;&lt;div&gt;on ➢❲dev❳ {⟪result = re.sub (r'%5C#.*%5Cn', '%5Cn', message.data)⟫}&lt;/div&gt;&lt;/div&gt;&lt;/div&gt;&lt;/div&gt;&lt;/div&gt;&lt;div&gt;&lt;br&gt;&lt;/div&gt;").
diagram_fact(value, cell_6, "dev").
diagram_fact(value, cell_7, "filename").
diagram_fact(value, cell_8, "iterate").
diagram_fact(value, cell_9, "directory").
diagram_fact(vertex, cell_10, 1).
diagram_fact(vertex, cell_6, 1).
diagram_fact(vertex, cell_7, 1).
diagram_fact(vertex, cell_8, 1).
diagram_fact(vertex, cell_9, 1).
diagram_fact(verticalAlign, cell_10, "top").
diagram_fact(verticalAlign, cell_6, "top").
diagram_fact(whiteSpace, cell_10, "wrap").
diagram_fact(whiteSpace, cell_6, "wrap").
diagram_fact(whiteSpace, cell_7, "wrap").
diagram_fact(whiteSpace, cell_8, "wrap").
diagram_fact(whiteSpace, cell_9, "wrap").
diagram_fact(width, cell_10, 335).
diagram_fact(width, cell_6, 450).
diagram_fact(width, cell_7, 40).
diagram_fact(width, cell_8, 40).
diagram_fact(width, cell_9, 40).
diagram_fact(x, cell_10, 1285).
diagram_fact(x, cell_6, 1230).
diagram_fact(x, cell_7, 1660).
diagram_fact(x, cell_8, 1210).
diagram_fact(x, cell_9, 1280).
diagram_fact(y, cell_10, 80).
diagram_fact(y, cell_6, 47.5).
diagram_fact(y, cell_7, 140).
diagram_fact(y, cell_8, 140).
diagram_fact(y, cell_9, 30).
das_fact(kind,cell_10,rectangle).
das_fact(kind,cell_4,rectangle).
das_fact(kind,cell_5,rectangle).
das_fact(kind,cell_6,rectangle).
das_fact(kind,cell_7,ellipse).
das_fact(kind,cell_8,ellipse).
das_fact(kind,cell_9,ellipse).
das_fact(name, cell_10, "&lt;div&gt;&lt;div&gt;&lt;div&gt;&lt;div&gt;&lt;div&gt;initially {⟪self.dirname = ''⟫}&lt;/div&gt;&lt;div&gt;on ➢❲dev❳ {⟪result = re.sub (r'%5C#.*%5Cn', '%5Cn', message.data)⟫}&lt;/div&gt;&lt;/div&gt;&lt;/div&gt;&lt;/div&gt;&lt;/div&gt;&lt;div&gt;&lt;br&gt;&lt;/div&gt;").
das_fact(name, cell_6, "dev").
das_fact(name, cell_7, "filename").
das_fact(name, cell_8, "iterate").
das_fact(name, cell_9, "directory").
das_fact(color,cell_10,"red").
das_fact(color,cell_4,"-").
das_fact(color,cell_5,"-").
das_fact(color,cell_6,"-").
das_fact(color,cell_7,"yellow").
das_fact(color,cell_8,"green").
das_fact(color,cell_9,"green").
das_fact(bbL,cell_10,1285).
  das_fact(bbT,cell_10,80).
  das_fact(bbR,cell_10,1620).
  das_fact(bbB,cell_10,240).
das_fact(bbL,cell_6,1230).
  das_fact(bbT,cell_6,47.5).
  das_fact(bbR,cell_6,1680).
  das_fact(bbB,cell_6,260).
das_fact(bbL,cell_7,1660).
  das_fact(bbT,cell_7,140).
  das_fact(bbR,cell_7,1700).
  das_fact(bbB,cell_7,180).
das_fact(bbL,cell_8,1210).
  das_fact(bbT,cell_8,140).
  das_fact(bbR,cell_8,1250).
  das_fact(bbB,cell_8,180).
das_fact(bbL,cell_9,1280).
  das_fact(bbT,cell_9,30).
  das_fact(bbR,cell_9,1320).
  das_fact(bbB,cell_9,70).
das_fact(direction,cell_7,output).
das_fact(direction,cell_8,input).
das_fact(direction,cell_9,input).
das_fact(contains,cell_6,cell_10).
das_fact(contains,cell_6,cell_7).
das_fact(contains,cell_6,cell_8).
das_fact(contains,cell_6,cell_9).
das_fact(direct_contains,cell_6,cell_10).
das_fact(direct_contains,cell_6,cell_7).
das_fact(direct_contains,cell_6,cell_8).
das_fact(direct_contains,cell_6,cell_9).
das_fact(codebox,cell_10,"&lt;div&gt;&lt;div&gt;&lt;div&gt;&lt;div&gt;&lt;div&gt;initially {⟪self.dirname = ''⟫}&lt;/div&gt;&lt;div&gt;on ➢❲dev❳ {⟪result = re.sub (r'%5C#.*%5Cn', '%5Cn', message.data)⟫}&lt;/div&gt;&lt;/div&gt;&lt;/div&gt;&lt;/div&gt;&lt;/div&gt;&lt;div&gt;&lt;br&gt;&lt;/div&gt;").
